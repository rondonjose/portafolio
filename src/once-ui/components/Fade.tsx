...
const Fade = forwardRef<HTMLDivElement, FadeProps>(
  (
    ...
    const getBaseVar = (base: BaseColor) => {
      ...
      // Validate base before using includes
      if (base && base.includes("alpha")) {
        const [scheme, weight] = base.split("-alpha-");

        return `var(--${scheme}-alpha-${weight})`;
      }

      const [scheme, weight] = base.split("-");

      return `var(--${scheme}-background-${weight})`;
    };
